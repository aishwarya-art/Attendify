...
const Calendar: React.FC<CalendarProps> = ({ attendanceData, selectedDate, holidays, onDateClick, updateStats, fetchHolidays }) => {
  ...
  const calculateMonthlyStats = (days: (Date | null)[], attendanceData: AttendanceRecord[], holidays: HolidayRecord[], today: Date, year: number, month: number) => {
    const workingDays = days.filter(day => day && day.getDay() !== 0 && !holidays.some(holiday => holiday.date === day.toISOString().split('T')[0]) && (day <= today || (year !== today.getFullYear() || month !== today.getMonth()))).length;
    ...
    const holidayDays = days.filter(day => {
      if (day) {
        ...
        return holidays.some(holiday => holiday.date === dayString) && (day <= today || (year !== today.getFullYear() || month !== today.getMonth()));
      }
      return false;
    }).length;
    ...
    <View style={styles.calendarContainer}>
      ...
      <View style={styles.calendarGrid}>
        ...
        {daysInMonth.map((day, index) => {
          ...
          const holidayName = holidays.find(holiday => holiday.date === day?.toISOString().split('T')[0])?.name;
          ...
          return (
            ...
            >
              ...
              {status === 'holiday' && <View style={styles.dotHoliday}><Text style={styles.holidayText}>{holidayName}</Text></View>}
