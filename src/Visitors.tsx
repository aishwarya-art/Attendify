...
import { View, Text, Image, StyleSheet,Dimensions, ScrollView, TouchableOpacity, Modal, Button, Alert } from 'react-native';
...
import { FaPhone, FaEnvelope, FaHome, FaPowerOff, FaClock,FaChevronRight, FaChevronLeft, FaArrowLeft, FaRegClock,FaUserTie,FaUser,FaBriefcase,FaBell,FaUserFriends,FaWalking,FaUsers } from 'react-icons/fa';
...
const Visitors: React.FC = () => {
  ...
  useEffect(() => {
    const fetchVisitorPendingData = async () => {
      const user_id = await AsyncStorage.getItem('cid');
      try {
        const response = await axios.get(`https://dev.techkshetra.ai/office_webApi/public/index.php/Guest/Guest_Pending_status?user_id=${user_id}`);
        setPendingList(response.data.data);
        setVisitorPendingCount(response.data.data.length); // Set visitor count
        console.log("Visitor pending data fetched:", response.data.data);
      } catch (error) {
        console.error('Error fetching pending data:', error);
      }
    };

    fetchVisitorPendingData();
    const intervalId = setInterval(fetchVisitorPendingData, 3000);
  ...
  const handleNextPendingPage = () => {
    if ((currentPendingPage + 1) * itemsPerPage < PendingList.length) {
      setCurrentPendingPage(currentPendingPage + 1);
    }
  };
    ...
    useEffect(() => {
      const showSubscription = Keyboard.addListener('keyboardDidShow', handleInputFocus);
      const hideSubscription = Keyboard.addListener('keyboardDidHide', handleInputBlur);
  ...
  const visiblePending = PendingList.slice(currentPendingPage * itemsPerPage, (currentPendingPage + 1) * itemsPerPage);
  return (
    <div> 
    <View style={styles.wrapper}>
      ...
      <View style={styles.timeContainerdiv}>
        ...
        <Text><FaWalking/> <b>{totalvisitorCount}</b>   </Text>
            ...
            visibleVisitors.map((visitor, index) => (
              ...
              <TouchableOpacity key={index} onPress={() => handleCardClick(visitor)}>
                ...
                <View style={styles.info}>
                  ...
                  <Text style={styles.textTime}>{formatTime(visitor.first_check_in)}</Text>
                </View>
                </View>
              </TouchableOpacity>
...
{/*        
  ...
  <View style={styles.buttonsContainer}>
      <TouchableOpacity
        ...
        style={currentPage === 0 ? styles.disabledButton : styles.buttonPagination}
      >
        <FaChevronLeft style={currentPage === 0 ? styles.iconDisabled : styles.iconEnabled} />
      </TouchableOpacity>
      <TouchableOpacity
        onPress={handleNextPage}
        disabled={(currentPage + 1) * itemsPerPage >= data.length}
        style={(currentPage + 1) * itemsPerPage >= data.length ? styles.disabledButton : styles.buttonPagination}
      >
        <FaChevronRight style={(currentPage + 1) * itemsPerPage >= data.length ? styles.iconDisabled : styles.iconEnabled} />
      </TouchableOpacity>
    </View>
    <Text>Pending Status</Text>
      <ScrollView>
        {visiblePending.length > 0 ? (
          visiblePending.map((pending, index) => (
            <TouchableOpacity key={index} onPress={() => openModal(pending)}>
              <View
                style={[
                  styles.visitorCard,
                  {
                    backgroundColor:
                      pending.status === 1 ? 'rgb(224, 233, 219)' : pending.status === 2 ? 'rgb(235, 224, 224)' : '#fff',
                  },
                ]}
              >
                <Image
                  style={styles.avatar}
                  source={{ uri: `https://vision.techkshetra.ai/faceRecognitionEngine/application/uploads/${pending.guest_photo}` }}
                />
                <View style={styles.info}>
                  <View style={styles.dateStatusRow}>
                    <Text style={styles.visitorTitle}>{pending.first_name}</Text>
                    <Text
                      style={[
                        styles.textStatus,
                        { color: pending.status === 1 ? 'green' : pending.status === 2 ? 'red' : 'black' },
                      ]}
                    >
                      <b>{pending.status === 1 ? 'Approved' : pending.status === 2 ? 'Rejected' : 'Pending'}</b>
                    </Text>
                  </View>
                  <View style={styles.dateStatusRow}>
                    <View>
                      <Text style={styles.textdate}>{formatDate(pending.first_check_in)}</Text>
                    </View>
                    <View>
                      <Text style={styles.textTime}>{formatTime(pending.first_check_in)}</Text>
                    </View>
                  </View>
                </View>
              </View>
      ...
      <View style={styles.buttonsContainer}>
        <TouchableOpacity
          onPress={handlePreviousPendingPage}
          disabled={currentPendingPage === 0}
          style={currentPendingPage === 0 ? styles.disabledButton : styles.buttonPagination}
        >
          <FaChevronLeft style={currentPendingPage === 0 ? styles.iconDisabled : styles.iconEnabled} />
        </TouchableOpacity>
        <TouchableOpacity
          onPress={handleNextPendingPage}
          disabled={(currentPendingPage + 1) * itemsPerPage >= PendingList.length}
          style={(currentPendingPage + 1) * itemsPerPage >= PendingList.length ? styles.disabledButton : styles.buttonPagination}
        >
          <FaChevronRight style={(currentPendingPage + 1) * itemsPerPage >= PendingList.length ? styles.iconDisabled : styles.iconEnabled} />
         ...
         <ScrollView>
          ...
          <View style={styles.dateStatusRow}>
            <View>
              ...
              <View>
              <Text style={styles.textTime}>{formatTime(guest.first_check_in)}</Text>
              </View>
            </View>
...
<View style={styles.buttonsContainer}>
        <TouchableOpacity
          onPress={handlePreviousGuestPage}
          disabled={currentGuestPage === 0}
          style={currentGuestPage === 0 ? styles.disabledButton : styles.buttonPagination}
        >
          <FaChevronLeft style={currentGuestPage === 0 ? styles.iconDisabled : styles.iconEnabled} />
        </TouchableOpacity>
        <TouchableOpacity
          onPress={handleNextGuestPage}
          disabled={(currentGuestPage + 1) * itemsPerPage >= guestList.length}
          style={(currentGuestPage + 1) * itemsPerPage >= guestList.length ? styles.disabledButton : styles.buttonPagination}
        >
          <FaChevronRight style={(currentGuestPage + 1) * itemsPerPage >= guestList.length ? styles.iconDisabled : styles.iconEnabled} />
        </TouchableOpacity>
      </View>
        </ScrollView>
        {selectedVisitor && (
         ...
         <ScrollView contentContainerStyle={styles.modalScrollContainer}>
           <View style={styles.modalHeader}>
             <Image
               ...
               source={{ uri: `https://vision.techkshetra.ai/faceRecognitionEngine/application/uploads/${selectedVisitor.guest_photo}` }}
             />
           </View>
  ...
  >
          ...
          <View style={styles.historyTable}>
            {visitorHistory.length > 0 ? (
              ...
              <View style={styles.noVisitorsCard}>
                <Text style={styles.historyCell} >No history!</Text>
              </View>
            )}
          </View>
        </View>
    ...
    <div className="footer">
      <FaBell className="footer-icon"  onClick={() => navigate('/Guest')}/>
