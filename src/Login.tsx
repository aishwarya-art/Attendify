...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    const checkUserLoggedIn = async () => {
      ...
      if (userId) {
        const level_id = await AsyncStorage.getItem('level_id');
        if (level_id === '7') {
          navigate('/Visitors');
        } else if (level_id === '6') {
    ...
    try {
      ...
      if (response.data.status) {
        const userDetails = {
          ...
          shift_duration: response.data.shift_duration,
        ...
        if (userDetails.level_id === '7') {
          navigate('/Visitors');
        } else if (userDetails.level_id === '6') {
  ...
  return (
    <div className="login-container">
      <div className="login-box">
        ...
        <img src={'https://techkshetra.ai/Attendify/images/attendify_logo4.png'} alt="Logo" className="login-logo" /> 
        {alertMessage && (
